...
from fraud_dynamic_ensemble.config import EXTERNAL_DATA_DIR
...
@app.command()
def main(
    output_path: Path = EXTERNAL_DATA_DIR / "creditcardfraud.csv",
) -> None:
    """
    ...
    (``creditcard.csv``) is present in the downloaded directory, creates ``EXTERNAL_DATA_DIR`` if needed,
    and copies the file to ``output_path``. Progress and outcomes are logged.

    Parameters
    ----------
    output_path : Path, optional
        Expected file output path of the raw dataset (default is ``EXTERNAL_DATA_DIR / "creditcardfraud.csv"``).
    ...
    logger.success("Running fraud_dynamic_ensemble/dataset.py completed successfully!")
